...
namespace SJ
{
	...
	private:
		...
		int m_slot = 0;
	public:
		Texture(std::filesystem::path filepath, int wrapping); //!< Load texture from file path
		...
		void reloadTexture(std::filesystem::path filepath, int wrapping);//!< Reload texture with a different image
